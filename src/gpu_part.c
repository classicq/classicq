...
// classic + QMB particles and flashblend dlight bubbles for the SDL_GPU path
// ported from gl_part.c, gl_rpart.c and gl_rlight.c
...
static int run_pipe = -1;
static int run_tex;
static unsigned int run_firstindex;
static unsigned int run_numindices;

static void Run_Flush(void)
{
	if (run_numindices)
		Scene_AddBatch(run_pipe, run_tex, NULL, GPU_GetDynamicSceneVB(), run_firstindex, run_numindices, r_viewproj);
	run_numindices = 0;
	run_pipe = -1;
}

static void Run_Select(int pipe, int tex)
{
	if (pipe != run_pipe || tex != run_tex)
	{
		Run_Flush();
		run_pipe = pipe;
		run_tex = tex;
...
static unsigned int *Run_Indices(unsigned int count)
{
	unsigned int firstindex;
	unsigned int *idx;

	idx = Scene_AllocIndices(count, &firstindex);
	if (!idx)
		return NULL;
	if (!run_numindices)
		run_firstindex = firstindex;
	run_numindices += count;
	return idx;
}

// untextured triangle fan, center + ring
static void Run_EmitFan(const vec3_t center, const byte ccol[4], const byte ecol[4], vec3_t *edges, int numedges)
{
	scene_vert_t *v;
	unsigned int *idx;
	...
	v = Scene_AllocVerts(1 + numedges, &firstvert);
	if (!v)
		return;
	idx = Run_Indices(3 * (numedges - 1));
	if (!idx)
		return;

	VectorCopy(center, v[0].pos);
	memcpy(v[0].rgba, ccol, 4);
	for (j = 0; j < numedges; j++)
	{
		VectorCopy(edges[j], v[1 + j].pos);
		memcpy(v[1 + j].rgba, ecol, 4);
	}
	for (j = 0; j < 1 + numedges; j++)
	{
		v[j].st[0] = v[j].st[1] = 0;
		v[j].lm[0] = v[j].lm[1] = 0;
	}
	...
	{
		idx[j*3+0] = firstvert;
		idx[j*3+1] = firstvert + 1 + j;
		idx[j*3+2] = firstvert + 2 + j;
	}
}

// texrect = {s1, t1, s2, t2}; uv order matches the GL quad
static void Run_EmitQuad(vec3_t *corners, const float *texrect, const byte col[4])
{
	scene_vert_t *v;
	unsigned int *idx;
	unsigned int firstvert;
	...
	for (j = 0; j < 4; j++)
	{
		VectorCopy(corners[j], v[j].pos);
		memcpy(v[j].rgba, col, 4);
		v[j].lm[0] = v[j].lm[1] = 0;
	}
	v[0].st[0] = texrect[0]; v[0].st[1] = texrect[3];
	v[1].st[0] = texrect[0]; v[1].st[1] = texrect[1];
	v[2].st[0] = texrect[2]; v[2].st[1] = texrect[1];
	v[3].st[0] = texrect[2]; v[3].st[1] = texrect[3];
...
// ---- classic particles (gl_part.c) ----

static float r_partscale;
static vec3_t part_up, part_right;
static unsigned int part_firstvert;
...
void GL_DrawParticleBegin(void)
{
	...
	VectorScale(vup, 1.5, part_up);
	VectorScale(vright, 1.5, part_right);
...
void GL_DrawParticle(particle_t *p)
{
	...
	v = Scene_AllocVerts(3, &firstvert);
	if (!v)
	{
		part_full = true;
		return;
	...
	VectorCopy(p->org, v[0].pos);
	v[0].st[0] = 0; v[0].st[1] = 0;

	VectorMA(p->org, scale, part_up, v[1].pos);
	v[1].st[0] = 1; v[1].st[1] = 0;

	VectorMA(p->org, scale, part_right, v[2].pos);
	v[2].st[0] = 0; v[2].st[1] = 1;
...
void GL_DrawParticleEnd(void)
{
	...
	idx = Scene_AllocIndices(part_numverts, &firstindex);
	if (!idx)
		return;
	for (i = 0; i < part_numverts; i++)
		idx[i] = part_firstvert + i;

	Scene_AddBatch(SCENE_PIPE_TEX_BLEND, particletexture, NULL, GPU_GetDynamicSceneVB(), firstindex, part_numverts, r_viewproj);
}

// ---- QMB particles (gl_rpart.c) ----
...
// tag and typedef renamed from particle_s, the classic particle_t is in scope here
...
int QMB_InitParticles(void)
{
	...
	particles = malloc(r_numparticles * sizeof(qmb_particle_t));
	if (particles)
		return 1;
...
static void QMB_UpdateParticles(void) {
	...
	particle_type_t *pt;
	qmb_particle_t *p;
	qmb_particle_t *prevp;
	qmb_particle_t *nextp;
	...
	for (i = 0; i < num_particletypes; i++) {
		...
		prevp = ((void *)pt) + (((void*)&((particle_type_t *)0)->start) - ((void *)&((qmb_particle_t *)0)->next)) ;
		nextp = pt->start;
		while((p = nextp))
		{
			nextp = p->next;
...
static int QMB_PipeForBlend(int SrcBlend, int DstBlend)
{
	if (SrcBlend == GL_ONE && DstBlend == GL_ONE)
		return SCENE_PIPE_ADD_NODEPTHWRITE;
	if (SrcBlend == GL_ZERO && DstBlend == GL_ONE_MINUS_SRC_COLOR)
		return SCENE_PIPE_INVSRCCOLOR_NODEPTHWRITE;
	if (SrcBlend == GL_SRC_ALPHA && DstBlend == GL_ONE_MINUS_SRC_ALPHA)
		return SCENE_PIPE_TEX_BLEND_NODEPTHWRITE;
	return SCENE_PIPE_ADDALPHA_NODEPTHWRITE;
}

static void QWB_DrawBillboardParticle(particle_texture_t *ptex, qmb_particle_t *p, vec3_t *coord)
{
	vec3_t newcoord[4];
	int j;

	for (j = 0; j < 4; j++)
		VectorMA(p->org, p->size, coord[j], newcoord[j]);

	Run_EmitQuad(newcoord, ptex->coords[p->texindex], p->color);
}

static void QWB_DrawBillboardParticleRotate(particle_texture_t *ptex, qmb_particle_t *p, vec3_t *coord, vec3_t vpn)
{
	vec3_t rotated, newcoord[4];
	int j;

	for (j = 0; j < 4; j++)
	{
		RotatePointAroundVector(rotated, vpn, coord[j], p->rotangle);
		VectorMA(p->org, p->size, rotated, newcoord[j]);
	}
...
void QMB_DrawParticles (void) {
	...
	for (i = 0; i < num_particletypes; i++) {
		...
		pipe = QMB_PipeForBlend(pt->SrcBlend, pt->DstBlend);

		switch(pt->drawtype) {
		case pd_spark:
			Run_Select(pipe, GPU_Texture_White());
			for (p = pt->start; p; p = p->next) {
				if (particle_time < p->start || particle_time >= p->die)
					continue;

				edgecolor[0] = p->color[0] >> 1;
				edgecolor[1] = p->color[1] >> 1;
				edgecolor[2] = p->color[2] >> 1;
				edgecolor[3] = 0;
				for (j = 7; j >= 0; j--) {
					for (k = 0; k < 3; k++)
						v[k] = p->org[k] - p->vel[k] / 8 + vright[k] * cost[j % 7] * p->size + vup[k] * sint[j % 7] * p->size;
					VectorCopy(v, edges[7 - j]);
				}
				Run_EmitFan(p->org, p->color, edgecolor, edges, 8);
			}
			break;
		case pd_sparkray:
			Run_Select(pipe, GPU_Texture_White());
			for (p = pt->start; p; p = p->next) {
				...
				Run_EmitFan(p->endorg, p->color, edgecolor, edges, 8);
			}
			break;
		case pd_billboard:
			ptex = &particle_textures[pt->texture];
			Run_Select(pipe, ptex->texnum);
			drawncount = 0;
			for (p = pt->start; p; p = p->next) {
				if (particle_time < p->start || particle_time >= p->die)
					continue;
...
// ---- flashblend dlight bubbles (gl_rlight.c) ----

static float bubble_sintable[17], bubble_costable[17];
static qboolean bubble_initialized;
...
static void R_RenderDlight(dlight_t *light)
{
	...
	VectorSubtract(light->origin, v, center);

	ccol[0] = bubblecolor[light->type][0] * 255;
	ccol[1] = bubblecolor[light->type][1] * 255;
	ccol[2] = bubblecolor[light->type][2] * 255;
	ccol[3] = 255;
	ecol[0] = ecol[1] = ecol[2] = 0;
	ecol[3] = 255;
	...
	{
		for (j = 0; j < 3; j++)
			edges[16 - i][j] = light->origin[j] + (v_right[j]*(*bub_cos) +
				+ v_up[j]*(*bub_sin)) * rad;
		bub_sin++;
		bub_cos++;
	}

	Run_Select(SCENE_PIPE_ADD_NODEPTHWRITE, GPU_Texture_White());
	Run_EmitFan(center, ccol, ecol, edges, 17);
}
