...
#include "shaders_gen.h"	// generated by build.zig; zig build shaders recompiles the blobs
...
int GPU_ReadPixels(unsigned char *rgb, unsigned int width, unsigned int height)
{
	...
	// mid-frame: flush the open cmdbuf so the shot has this frame, skipping its presentation
	if (frame_cmdbuf)
	{
		record_scene_pass();
		cmdbuf = frame_cmdbuf;
